-- ===================================
-- 🧹 LIMPAR E INSERIR PENDÊNCIAS FINANCEIRAS
-- ===================================
-- Execute este script depois do DATABASE_SETUP.sql

-- Primeiro, remover duplicatas existentes mantendo apenas uma entrada por (nome, ano)
WITH duplicates AS (
    SELECT id, 
           ROW_NUMBER() OVER (PARTITION BY nome, ano ORDER BY created_at) as rn
    FROM despesas_mensais
)
DELETE FROM despesas_mensais 
WHERE id IN (
