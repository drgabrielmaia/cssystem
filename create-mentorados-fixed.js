...
async function createOrUpdateMentorado(mentoradoData, organizationId) {
  ...
  try {
    ...
    if (existing) {
      // Atualizar existente (SEM updated_at)
      ...
      // Criar novo (com status_login correto)
      ...
      const mentoradoPayload = {
        ...
        estado_entrada: 'novo',
        estado_atual: 'ativo',
        status_login: 'ativo', // Mudando de 'permitido' para 'ativo'
        excluido: false,
      ...
      if (createResponse.ok) {
        ...
        // Tentar com status_login diferente se o primeiro falhar
        if (error.includes('status_login_check')) {
          console.log(`🔄 Tentando novamente com status_login='liberado'...`);
          mentoradoPayload.status_login = 'liberado';
          ...
          if (retryResponse.ok) {
            const retryCreated = await retryResponse.json();
            console.log(`✅ Mentorado criado (retry): ${name}`);
            return { success: true, action: 'created', data: retryCreated };
          }
        }
...
async function main() {
  console.log('🚀 INICIANDO PROCESSAMENTO DE MENTORADOS (VERSÃO CORRIGIDA)');
  ...
  if (errors === 0) {
    ...
    console.log('✅ Todos os mentorados foram associados à organização temp2@admin.com');
    console.log('🔑 Status de login configurado como ativo/liberado');
  } else {
    console.log('\n⚠️ Processamento concluído com alguns erros.');
  }

  console.log('\n📋 PRÓXIMOS PASSOS:');
  console.log('1. 🔑 Todos têm acesso básico ao portal');
  console.log('2. 📧 Considerar envio de credenciais de acesso');
  console.log('3. 🎯 Configurar metas iniciais se necessário');
  console.log('4. 🎥 Acesso ao sistema de vídeos já configurado');
}
