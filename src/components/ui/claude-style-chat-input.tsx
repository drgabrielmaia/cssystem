...
const FilePreviewCard: React.FC<FilePreviewCardProps> = ({ file, onRemove }) => {
    ...
    return (
        <div className={`relative group flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border border-gray-300 bg-gray-100 transition-all hover:border-gray-400`}>
            {isImage ? (
                ...
                <div className="w-full h-full p-3 flex flex-col justify-between">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-gray-200 rounded">
                            <Icons.FileText className="w-4 h-4 text-gray-600" />
                        </div>
                        <span className="text-[10px] font-medium text-gray-600 uppercase tracking-wider truncate">
                            {file.file.name.split('.').pop()}
                        </span>
                    </div>
                    <div className="space-y-0.5">
                        <p className="text-xs font-medium text-gray-800 truncate" title={file.file.name}>
                            {file.file.name}
                        </p>
...
const PastedContentCard: React.FC<PastedContentCardProps> = ({ content, onRemove }) => {
    return (
        <div className="relative group flex-shrink-0 w-28 h-28 rounded-2xl overflow-hidden border border-gray-200 bg-white p-3 flex flex-col justify-between shadow-sm">
            <div className="overflow-hidden w-full">
                <p className="text-[10px] text-gray-500 leading-[1.4] font-mono break-words whitespace-pre-wrap line-clamp-4 select-none">
                    {content.content}
                </p>
            </div>

            <div className="flex items-center justify-between w-full mt-2">
                <div className="inline-flex items-center justify-center px-1.5 py-[2px] rounded border border-gray-200 bg-white">
                    <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider font-sans">PASTED</span>
                </div>
            </div>

            <button
                onClick={() => onRemove(content.id)}
                className="absolute top-2 right-2 p-[3px] bg-white border border-gray-200 rounded-full text-gray-500 hover:text-gray-700 transition-colors shadow-sm opacity-0 group-hover:opacity-100"
            >
...
const ModelSelector: React.FC<ModelSelectorProps> = ({ models, selectedModel, onSelect }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute bottom-full right-0 mb-2 w-[260px] bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col p-1.5">
                    {models.map(model => (
                        <button
                            key={model.id}
                            onClick={() => {
...
export const ClaudeChatInput: React.FC<ClaudeChatInputProps> = ({ onSendMessage }) => {
    ...
    const [selectedModel, setSelectedModel] = useState("motivacional");
    ...
    const models = [
        { id: "motivacional", name: "Post Motivacional", description: "Cria posts motivacionais envolventes" },
        { id: "educativo", name: "Post Educativo", description: "Conteúdo educativo e informativo" },
        { id: "promocional", name: "Post Promocional", description: "Posts para promoções e vendas" },
        { id: "stories", name: "Stories", description: "Conteúdo para Instagram Stories" },
        { id: "carrossel", name: "Carrossel", description: "Posts em formato de carrossel" }
    ];
    ...
    const handleFiles = useCallback((newFilesList: FileList | File[]) => {
        ...
        if (!message && newFiles.length === 1) {
            const f = newFiles[0];
            if (f.type.startsWith('image/')) {
                setMessage("Crie um post baseado nesta imagem...");
            } else {
                setMessage("Analise este documento e crie um post...");
            }
        }
    ...
    const handlePaste = (e: React.ClipboardEvent) => {
        ...
        if (text.length > 300) {
            ...
            if (!message) {
                setMessage("Transforme este texto em um post...");
            }
        }
    };
    ...
    return (
        ...
        >
            <div className={`
                !box-content flex flex-col mx-2 md:mx-0 items-stretch transition-all duration-200 relative z-10 rounded-2xl cursor-text border border-gray-200
                    ...
                    <div className="flex gap-2 w-full items-center">
                        <div className="relative flex-1 flex items-center shrink min-w-0 gap-1">
                            ...
                            <button
                                ...
                                className="inline-flex items-center justify-center relative shrink-0 transition-colors duration-200 h-8 w-8 rounded-lg active:scale-95 text-gray-500 hover:text-gray-700 hover:bg-gray-100"
                                type="button"
                            ...
                            <div className="flex shrink min-w-8 !shrink-0">
                                ...
                                >
                                    ...
                                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-2 py-1 bg-gray-800 text-white text-[11px] font-medium rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50 flex items-center gap-1 shadow-sm tracking-wide">
            ...
            {isDragging && (
                <div className="absolute inset-0 bg-gray-50/90 border-2 border-dashed border-orange-400 rounded-2xl z-50 flex flex-col items-center justify-center backdrop-blur-sm pointer-events-none">
                    <Icons.Archive className="w-10 h-10 text-orange-500 mb-2 animate-bounce" />
                    <p className="text-orange-600 font-medium">Solte os arquivos aqui</p>
                </div>
            )}
