...
-- Note: The foreign key to closers table uses UUID type for id to match the existing 
...
CREATE TABLE IF NOT EXISTS closer_study_materials (
    ...
    created_by UUID REFERENCES closers(id) ON DELETE SET NULL, -- Using UUID to match closers table
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS closer_material_progress (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID REFERENCES closers(id) ON DELETE CASCADE, -- Using UUID to match closers table
